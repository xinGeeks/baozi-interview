"""面试页:对话循环 + 逐轮反馈 + 输入框 + 专项训练退出。

流程第 2 页。到达时若 pending_start 为真则 auto-start 生成第一题。
END_SIGNAL / 退出专项训练 / 结束按钮 → 生成报告 → 跳报告页。
"""
...
# ---- auto-start:从配置页 / 训练图谱页带 pending_start 进来 ----
if (
...
if st.session_state.practice_mode:
    st.title("🎯 专项训练")
    st.caption(f"焦点主题:「{st.session_state.practice_topic}」")
else:
    st.title("💬 面试对话")
    st.caption(
...
if not st.session_state.interview_ended:
    if st.session_state.practice_mode:
        if st.button("🚪 退出专项训练", key="exit_practice", type="secondary"):
            _generate_report()
...
if st.session_state.interview_started and not st.session_state.interview_ended:
    ...
    user_input = st.chat_input(
        ...
        + (
            " 或『退出专项训练』可结束本次专项训练"
            if st.session_state.practice_mode else ""
        ),
        disabled=_budget_blocked,
    )
    if user_input and not _budget_blocked:
        should_end = END_SIGNAL in user_input or (
            st.session_state.practice_mode and "退出专项训练" in user_input
        )
        _handle_user_answer(user_input, generate_next=not should_end)
        if END_SIGNAL in user_input:
            _generate_report()
            st.session_state.pending_report_nav = True
        elif st.session_state.practice_mode and "退出专项训练" in user_input:
            _generate_report()
            st.session_state.practice_mode = False
            st.session_state.practice_topic = ""
            st.session_state.pending_report_nav = True

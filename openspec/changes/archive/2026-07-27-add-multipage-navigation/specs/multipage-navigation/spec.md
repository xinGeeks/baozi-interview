...
### Requirement: Sidebar is global and only shows ToS / privacy / data deletion

The system MUST keep the sidebar global across all 4 pages and only render these items:
- A ToS status indicator (e.g., `✅ 已接受 ToS v<version>` or `⚠️ 未接受 ToS`).
- A link or button to "查看完整隐私政策".
...
The ToS gate (first-time modal acceptance) MUST remain in `app.py` entry point, before `st.navigation` is invoked, so that no page renders until ToS is accepted.

#### Scenario: Sidebar shows only global items

- **WHEN** the user is on any of the 4 pages
- **THEN** the sidebar contains only the ToS status, privacy link, and 清空我的全部历史 button (verified by absence of `🎯 跨会话训练图谱` and `🎯 弱 topic 专项练习` expander labels in sidebar)

#### Scenario: ToS gate blocks all pages until accepted

- **WHEN** the user has not accepted the current `TOS_VERSION` for the current `candidate_id`
- **THEN** `app.py` invokes `st.stop()` before `st.navigation` so none of the 4 pages render; the ToS modal is displayed

#### Scenario: After ToS accepted, all 4 pages accessible

- **WHEN** `st.session_state.tos_accepted` is `True`
- **THEN** the user can navigate to any of the 4 pages
